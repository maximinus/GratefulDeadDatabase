...
function getRarestSongs() {
    year_of_show = show_store.current_shown_show.js_date.getFullYear()
...
function getShowRenderData() {
    ...
    return {'show-date': convertDateLong(show_store.current_show.date),
            'show-venue': store.venues[show_store.current_show.venue].getVenueName(),
            'sets': sets_all_data};
};
...
function displayShow(show_index) {
    ...
    log(`Rendering show ${store.shows[show_index].js_date}`);
